...
Workspace switching preserves work, stops relevant playback and hides key reveal. Hiding the browser document remasks the key and suppresses pending clipboard feedback; returning keeps it masked until another explicit Reveal. This visibility handler preserves unfinished notes, the finalized envelope, prepared file/key binding, acknowledgement and pending crypto. Masking does not lock the session or erase the key. `pagehide`, unmount and persisted `pageshow` discard app references best-effort; a reload requires the file and key again. Before-unload warnings cannot guarantee cleanup on abrupt termination.
...
[Separate key-display correction checks and preservation evidence](sealed-key-display-correction/README.md) cover explicit reveal/copy behavior and nondestructive visibility remasking.
